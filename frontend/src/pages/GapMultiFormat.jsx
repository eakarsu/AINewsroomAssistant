...
// Multi Format
// Multi-format content generation (clips, scripts, podcasts, infographics) from single story

export default function GapMultiFormat() {
  ...
  const runAnalysis = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/gap-multi-format', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 960, margin: '0 auto' }}>
      <h1 style={{ fontSize: 28, marginBottom: 6 }}>Multi Format</h1>
      <p style={{ color: '#555', marginBottom: 20 }}>Multi-format content generation (clips, scripts, podcasts, infographics) from single story</p>
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 8, padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ marginTop: 12, display: 'flex', gap: 8 }}>
          ...
          >
            {loading ? 'Running...' : 'Run Multi Format'}
          </button>
          <button
