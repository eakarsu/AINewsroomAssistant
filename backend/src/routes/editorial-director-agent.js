// Auto-generated feature route: editorial-director-agent
// Domain: agentic editorial director — Capability: planning topics + sources + assignments from trends
// Mounted under: /api/editorial-director-agent
...
// POST /api/editorial-director-agent/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a agentic editorial director. Your task is planning topics + sources + assignments from trends. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/editorial-director-agent/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/editorial-director-agent/:id — fetch one
...
// DELETE /api/editorial-director-agent/:id — discard a run
