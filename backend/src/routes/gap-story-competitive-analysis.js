...
// Route: /api/gap-story-competitive-analysis
// Feature: Story Competitive Analysis
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Story Competitive Analysis. (competitor coverage)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Story Competitive Analysis\nDescription: (competitor coverage)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['story-competitive-analysis', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'story-competitive-analysis', title: 'Story Competitive Analysis', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'story-competitive-analysis', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['story-competitive-analysis']);
      return res.json({ slug: 'story-competitive-analysis', items: r.rows || [] });
    }
    return res.json({ slug: 'story-competitive-analysis', items: [] });
  } catch (e) {
    return res.json({ slug: 'story-competitive-analysis', items: [], error: (e && e.message) || 'Server error' });
  }
});
