...
// Route: /api/gap-publishing
// Feature: Publishing
// Description: Publishing CMS integration (WordPress, Arc, custom)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Publishing. Publishing CMS integration (WordPress, Arc, custom)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'publishing', title: 'Publishing', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'publishing', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['publishing']);
      return res.json({ slug: 'publishing', items: r.rows || [] });
    }
    return res.json({ slug: 'publishing', items: [] });
  } catch (e) {
    return res.json({ slug: 'publishing', items: [], error: (e && e.message) || 'Server error' });
  }
});
