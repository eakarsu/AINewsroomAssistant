...
// Route: /api/gap-reader
// Feature: Reader
// Description: Reader engagement analytics
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Reader. Reader engagement analytics' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'reader', title: 'Reader', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'reader', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['reader']);
      return res.json({ slug: 'reader', items: r.rows || [] });
    }
    return res.json({ slug: 'reader', items: [] });
  } catch (e) {
    return res.json({ slug: 'reader', items: [], error: (e && e.message) || 'Server error' });
  }
});
