// Auto-generated feature route: multi-format-content
// Domain: multi-format content agent — Capability: producing clips, scripts, podcasts, infographics from a single story
// Mounted under: /api/multi-format-content
...
// POST /api/multi-format-content/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a multi-format content agent. Your task is producing clips, scripts, podcasts, infographics from a single story. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/multi-format-content/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/multi-format-content/:id — fetch one
...
// DELETE /api/multi-format-content/:id — discard a run
