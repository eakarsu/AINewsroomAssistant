// Auto-generated feature route: source-credibility-stream
// Domain: streaming source credibility scorer — Capability: tracking bias and accuracy history
// Mounted under: /api/source-credibility-stream
...
// POST /api/source-credibility-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a streaming source credibility scorer. Your task is tracking bias and accuracy history. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
    ...
    const record = {
      ...
      slug: 'source-credibility-stream',
      input: payload,
      output: parsed,
...
// GET /api/source-credibility-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/source-credibility-stream/:id — fetch one
...
// DELETE /api/source-credibility-stream/:id — discard a run
