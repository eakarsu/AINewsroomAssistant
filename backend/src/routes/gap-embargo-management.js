...
// Route: /api/gap-embargo-management
// Feature: Embargo Management
// Description: (embargo deadline tracking)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Embargo Management. (embargo deadline tracking)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Embargo Management\nDescription: (embargo deadline tracking)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['embargo-management', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'embargo-management', title: 'Embargo Management', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'embargo-management', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['embargo-management']);
      return res.json({ slug: 'embargo-management', items: r.rows || [] });
    }
    return res.json({ slug: 'embargo-management', items: [] });
  } catch (e) {
    return res.json({ slug: 'embargo-management', items: [], error: (e && e.message) || 'Server error' });
  }
});
