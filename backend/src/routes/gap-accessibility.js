...
// Route: /api/gap-accessibility
// Feature: Accessibility
// Description: Accessibility compliance (captions, alt-text)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Accessibility. Accessibility compliance (captions, alt-text)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    return res.json({ feature: 'accessibility', title: 'Accessibility', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'accessibility', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['accessibility']);
      return res.json({ slug: 'accessibility', items: r.rows || [] });
    }
    return res.json({ slug: 'accessibility', items: [] });
  } catch (e) {
    return res.json({ slug: 'accessibility', items: [], error: (e && e.message) || 'Server error' });
  }
});
