// Auto-generated feature route: bias-realtime-stream
// Domain: real-time bias + misinformation detector — Capability: running across drafts and published content
// Mounted under: /api/bias-realtime-stream
...
// POST /api/bias-realtime-stream/run — execute the feature
router.post('/run', async (req, res) => {
  try {
    const payload = req.body || {};
    const systemPrompt = 'You are a real-time bias + misinformation detector. Your task is running across drafts and published content. Respond with concise JSON: { "summary": string, "recommendations": string[], "next_actions": string[], "confidence": number }.';
...
// GET /api/bias-realtime-stream/history — recent runs
router.get('/history', (req, res) => {
  const limit = Math.min(parseInt(req.query.limit, 10) || 25, 100);
  res.json({ items: store.slice(-limit).reverse() });
});

// GET /api/bias-realtime-stream/:id — fetch one
...
// DELETE /api/bias-realtime-stream/:id — discard a run
