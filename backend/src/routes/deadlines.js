...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const countResult = await db.query('SELECT COUNT(*) FROM deadlines');
    const total = parseInt(countResult.rows[0].count);
    const result = await db.query('SELECT * FROM deadlines ORDER BY due_date ASC LIMIT $1 OFFSET $2', [limit, offset]);
