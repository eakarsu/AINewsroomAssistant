...
// Route: /api/gap-source-matching
// Feature: Source Matching
// Description: (match story to expert sources)
  ...
  try {
    const r = await fetch('https://openrouter.ai/api/v1/chat/completions', {
      ...
      body: JSON.stringify({
        ...
        messages: [
          { role: 'system', content: 'You are an expert assistant for: Source Matching. (match story to expert sources)' },
...
router.post('/', async (req, res) => {
  try {
    ...
    const prompt = `Feature: Source Matching\nDescription: (match story to expert sources)\nInput: ${input}\nContext: ${JSON.stringify(context || {})}`;
    const result = await callOpenRouter(prompt);
    // Best-effort persistence
    try {
      if (db && typeof db.query === 'function') {
        await db.query('INSERT INTO gap_features (slug, input, output, meta) VALUES ($1, $2, $3, $4)',
          ['source-matching', input, JSON.stringify(result), JSON.stringify(context || {})]);
    ...
    return res.json({ feature: 'source-matching', title: 'Source Matching', result, generatedAt: new Date().toISOString() });
  } catch (e) {
...
router.get('/history', async (req, res) => {
  try {
    const db = req.app && (req.app.get ? req.app.get('db') : null);
    if (!db) return res.json({ slug: 'source-matching', items: [] });
    if (typeof db.query === 'function') {
      const r = await db.query('SELECT id, input, output, created_at FROM gap_features WHERE slug = $1 ORDER BY id DESC LIMIT 20', ['source-matching']);
      return res.json({ slug: 'source-matching', items: r.rows || [] });
    }
    return res.json({ slug: 'source-matching', items: [] });
  } catch (e) {
    return res.json({ slug: 'source-matching', items: [], error: (e && e.message) || 'Server error' });
  }
});
